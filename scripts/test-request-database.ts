...
async function main() {
  ...
  try {
    ...
    const legacy = await import('../src/lib/requests/legacy-simple-advice-service')
    const { createIntake } = await import('../src/lib/intakes/intake-service')
    for (const primary of [null, 'HVK']) {
      const intake = await createIntake(owner.id, organization.id, { freeText: 'Dit is een bestaande lokale conceptopdracht.' })
      const beforeDossiers: number = await prisma.adviceDossier.count()
      const beforeRequests: number = await prisma.request.count()
      const prefill = await legacy.getLegacySimpleAdvice(simpleViewer, intake.id)
      assert.ok('initialValues' in prefill)
      assert.equal(await prisma.adviceDossier.count(), beforeDossiers)
      assert.equal(await prisma.request.count(), beforeRequests)
      await assert.rejects(legacy.getLegacySimpleAdvice({ ...simpleViewer, userId: otherOwner.id, organizationId: otherOrganization.id }, intake.id))
      const input = { ...basic, routeChoice: primary ? 'KNOWS_EXPERTISE' : 'NEEDS_TOPIC', requestedExpertise: primary, additionalExpertises: primary ? ['MVK', 'ARBEIDSHYGIENIST'] : [] }
      const version = await legacy.saveLegacySimpleAdvice(simpleViewer, intake.id, 0, input)
      await expectRequestError(() => legacy.saveLegacySimpleAdvice(simpleViewer, intake.id, 0, input), 'CONFLICT')
      await assert.rejects(prisma.intakeSimpleAdviceRevision.update({ where: { intakeId_version: { intakeId: intake.id, version } }, data: { payload: {} } }))
      const draftAssignment = primary ? await prisma.assignment.create({ data: { intakeId: intake.id, clientOrganizationId: organization.id, createdByUserId: owner.id, status: 'DRAFT', title: 'Bestaand concept', description: 'Oorspronkelijke conceptinhoud', revisions: { create: { version: 1, title: 'Bestaand concept', description: 'Oorspronkelijke conceptinhoud', allowsRemoteWork: false, changedByUserId: owner.id } } } }) : null
      const published = await legacy.publishLegacySimpleAdvice(simpleViewer, intake.id, version, input)
      if (draftAssignment) {
        const reused: Prisma.AssignmentGetPayload<{ include: { revisions: true } }> = await prisma.assignment.findUniqueOrThrow({ where: { id: draftAssignment.id }, include: { revisions: true } })
        assert.equal(reused.requestId, published.id)
        assert.equal(reused.intakeId, intake.id)
        assert.equal(reused.revisions.length, 2)
        assert.ok(reused.revisions.some(r => r.description === 'Oorspronkelijke conceptinhoud'))
      }
      assert.equal((await legacy.publishLegacySimpleAdvice(simpleViewer, intake.id, version, input)).id, published.id)
      const row: Prisma.RequestGetPayload<{ include: { assignment: true; assignmentHandoff: true } }> = await prisma.request.findUniqueOrThrow({ where: { id: published.id }, include: { assignment: true, assignmentHandoff: true } })
      assert.equal(row.adviceDossierId, intake.id)
      assert.equal(row.status, 'PUBLISHED')
      assert.equal(row.assignment?.status, 'OPEN')
      assert.equal(await prisma.assignment.count({ where: { requestId: row.id } }), 1)
      assert.equal(await prisma.requestAssignmentHandoff.count({ where: { requestId: row.id } }), 1)
      assert.equal(row.assignment?.responseDeadline?.toISOString(), responseDeadline(row.publishedAt!).toISOString())
      assert.deepEqual(row.additionalExpertiseCodes, input.additionalExpertises)
      assert.equal((row.assignmentHandoff!.snapshot as { matchingBlockReason: string | null }).matchingBlockReason, primary ? null : 'PRIMARY_EXPERTISE_REQUIRED')
      assert.equal((await prisma.intake.findUniqueOrThrow({ where: { id: intake.id } })).freeText, 'Dit is een bestaande lokale conceptopdracht.')
      assert.equal(await prisma.adviceDossier.count(), beforeDossiers + 1)
      assert.equal(await prisma.request.count(), beforeRequests + 1)
      await expectRequestError(() => legacy.saveLegacySimpleAdvice(simpleViewer, intake.id, version, input), 'NOT_ELIGIBLE')
    }
    const existingDossier = await createDossier('TEST-LEGACY-UNIFICATION')
    const originalVersion = await prisma.adviceDossierVersion.findFirstOrThrow({ where: { adviceDossierId: existingDossier.id } })
    const { startAdviceDossierIntake } = await import('../src/lib/advice-dossiers/advice-dossier-intake-handoff-service')
    const started = await startAdviceDossierIntake({ viewer: simpleViewer, dossierId: existingDossier.id })
    assert.equal(started.kind, 'INTAKE')
    if (started.kind !== 'INTAKE') throw Error('Expected draft')
    const dossierCount = await prisma.adviceDossier.count()
    const savedVersion = await legacy.saveLegacySimpleAdvice(simpleViewer, started.intakeId, 0, basic)
    const fromDossier = await legacy.publishLegacySimpleAdvice(simpleViewer, started.intakeId, savedVersion, basic)
    assert.equal((await prisma.request.findUniqueOrThrow({ where: { id: fromDossier.id } })).adviceDossierId, existingDossier.id)
    assert.equal(await prisma.adviceDossier.count(), dossierCount)
    assert.deepEqual(await prisma.adviceDossierVersion.findUniqueOrThrow({ where: { id: originalVersion.id } }), originalVersion)
    assert.equal(await prisma.adviceDossierVersion.count({ where: { adviceDossierId: existingDossier.id } }), 2)
    console.log('Legacy Simple Advice: read-only opening, prefill, append-only saves, concurrency, tenant, A/B canonical publication and repeat PASS.')
