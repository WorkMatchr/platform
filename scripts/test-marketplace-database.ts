...
async function main() {
  ...
  try {
    ...
    const account = await prisma.creditAccount.create({ data: { organizationId: providerOrganization.id } })
    await prisma.creditTransaction.create({ data: { creditAccountId: account.id, type: 'ADMIN_GRANT', amount: 10, totalDelta: 10, reservedDelta: 0, balanceAfter: 10, reason: 'Fictief beginsaldo voor de databasetest.', createdByUserId: providerUser.id, idempotencyKey: randomUUID() } })
    ...
    const ledger = await prisma.creditTransaction.create({ data: { creditAccountId: account.id, type: 'RESERVATION', amount: -10, totalDelta: 0, reservedDelta: 10, balanceAfter: 0, reservationId: reservation.id, reason: 'Fictieve reservering voor de databasetest.', createdByUserId: providerUser.id, idempotencyKey: randomUUID() } })
