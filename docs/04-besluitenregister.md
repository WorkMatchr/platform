...
| B-193 | Module 6B activeert het ADR-013-tenantcontract. | `OrganizationMembership.userId` is uniek; context wordt server-side zonder cookie of sessieclaim afgeleid; een tweede organisatie vereist een afzonderlijke User. De migratie kiest of herschrijft geen records en stopt bij conflicten. |
| B-194 | Contractactivatie voltooit geen accountverwijdering of membershipbeëindiging. | Retentie, purge, e-mailvrijgave en volledige offboarding blijven fail-closed totdat hun afzonderlijke juridische en operationele voorwaarden zijn bewezen. |
...
| B-242 | Voor nieuwe drafts kan `LEGACY_COMPATIBILITY` uitsluitend veilige shared context dragen; ieder vakspecifiek Context Goal vereist centraal geldige applicability plus een gepubliceerde routingregel en actuele gevalideerde claim. | De bovengrens van vijf vragen is geen streefgetal. Bij lage informatiewaarde of ontbrekende kennisdekking stopt de engine vroeg en fail-closed; historische 1.0-snapshots blijven hervatbaar. |
