...
Een factuur-PDF wordt server-side uitsluitend uit de immutable factuursnapshot opgebouwd, inclusief verkoper-, klant-, omschrijvings- en btw-gegevens. Daardoor verandert een historische PDF niet wanneer later organisatiegegevens wijzigen. Alleen een geauthenticeerd lid van dezelfde organisatie kan de beveiligde PDF-route bekijken of downloaden. Na een betaalde aankoop verstuurt WorkMatchr één factuurmail met een beveiligde downloadlink; een vaste Resend-idempotency key en append-only financiële delivery-events voorkomen dubbele verzending bij webhookreplay. Mislukte of verlopen betalingen maken en versturen geen factuur.
