...
Een betaalde Pro-first-payment kan de remote Mollie-subscription later activeren dan de oorspronkelijke webhookverwerking. Na zowel normale als herstelde activatie roept één begrensde downstream-finalizer uitsluitend de bestaande factuurdelivery en Jortt-sync aan voor de unieke betaalde Snapshot-v2-first-paymentfactuur. Beide stappen blijven onafhankelijk fail-closed en idempotent: een fout in één provider verhindert de andere downstreamstap niet, en replay of gelijktijdige aanroepen gebruiken de bestaande locks, provider-idempotency en append-only events zonder een nieuwe purchase, payment, factuur of subscription te maken.

Terugkerende Pro-facturen bepalen de canonieke first-paymentbron uit zowel de oorspronkelijke purchase als de immutable retrypogingen. Alleen één unieke betaalde Pro-purchase met factuur is toegestaan; failed pogingen zijn geen bron en meerdere betaalde kandidaten blijven fail-closed. De bestaande factuur wordt uitsluitend gelezen voor de klantsnapshot; historische gegevens worden niet gewijzigd.

Na commit van een betaalde recurring periode/factuur roept de webhook dezelfde factuurdelivery aan. Voor een recurring factuur wordt de ontvanger via de canonieke first-paymentmaker bepaald en de betaaldatum via de vastgelegde betaalde subscriptionperiode. De mail blijft onder dezelfde invoicegebonden advisory lock, provider-idempotency en succes-event vallen. Een mailfout draait de betaalde periode niet terug; normale webhookherhaling kan de delivery opnieuw proberen zonder een tweede factuur. De reeds bij issuance gemaakte Jortt-projectie wordt door de bestaande retry-maintenance verwerkt, onafhankelijk van maildelivery.
...
De remote-subscriptionfase is afzonderlijk, append-only observeerbaar en herstelbaar. Iedere poging krijgt onder een organisatiegebonden advisory lock een oplopend nummer via `PRO_REMOTE_SUBSCRIPTION_ATTEMPT_STARTED`. Lookup- en createfouten worden als `PRO_REMOTE_SUBSCRIPTION_LOOKUP_FAILED` respectievelijk `PRO_REMOTE_SUBSCRIPTION_CREATE_FAILED` vastgelegd met uitsluitend gesaniteerde providerdiagnostiek. Een geslaagde koppeling schrijft idempotent `PRO_REMOTE_SUBSCRIPTION_LINKED`. Een gecontroleerde retry gebruikt uitsluitend de reeds betaalde eerste aankoop en lokaal geverifieerde mandateprojectie, zoekt altijd eerst op de vaste interne subscription-identiteit en gebruikt bij create dezelfde stabiele Mollie-idempotentiesleutel. Daardoor ontstaan geen nieuwe purchase, payment, factuur, mandate of lokale subscription en wordt een remote subscription die tussen pogingen verscheen hergebruikt.

Bij het aanmaken van een Mollie-subscription geeft de transportadapter een expliciete `mandateId` zonder `method` door, omdat Mollie deze parameters als onderling exclusief behandelt. Alleen wanneer geen mandate-ID is opgegeven, gebruikt de adapter de bestaande betaalmethode als bronselectie.

Bij het teruglezen is het exact verwachte mandate-ID leidend. Een door Mollie weggelaten redundante methode is toegestaan; een expliciet aanwezige methode die strijdig is met de lokaal gevalideerde mandatecontext blijft fail-closed. Identiteit, organisatie, bedrag, valuta, interval en remote status blijven onverkort strikt gevalideerd.
...
De onderhoudsrunner voert refundreconciliatie, vervallen opzeggingen, achterstanden van minimaal één maand en Jortt-retries idempotent uit. Vercel Cron roept dagelijks om 03:00 UTC `GET /api/maintenance/finance` aan met het afzonderlijke `CRON_SECRET`; deze ingang bestaat uitsluitend in Production. De bestaande handmatige `POST`-ingang blijft afzonderlijk beschermd met `FINANCIAL_MAINTENANCE_SECRET`. Een databasebrede runlease voorkomt overlap. Iedere run registreert begin, einde, trigger, status, begrensde categorietotalen en uitsluitend veilige foutcodes. Een fout in één categorie verhindert de overige categorieën niet. Een afgebroken `RUNNING`-lease wordt na dertig minuten als mislukt geregistreerd voordat een nieuwe run kan starten.
...
De bestaande platformbeheer-beveiligde `GET /api/platformbeheer/financien/mollie/methoden-preflight` rapporteert naast de read-only oneoff-methodencontrole uitsluitend twee aanvullende configuratiebooleans: `redirectBaseUrlMatchesProduction` en `webhookBaseUrlMatchesProduction`. Deze vergelijken de ongewijzigde runtimewaarden exact met `https://www.workmatchr.nl`; ontbrekende waarden, andere hosts, whitespace en een trailing slash geven `false`. De onderliggende URL-waarden worden niet teruggegeven of gelogd. `ok` blijft het resultaat van de bestaande oneoff-methodencontrole. Een afzonderlijk `proFirstPayment`-resultaat controleert read-only de beschikbare methoden voor €59,29 met `sequenceType=first` en rapporteert fail-closed of minimaal één geschikte methode beschikbaar is. Voor Production-acceptatie moeten daarnaast beide URL-booleans `true` zijn. Autorisatie blijft vóór iedere controle staan en antwoorden blijven `private, no-store`. Deze route maakt geen customer, purchase, payment, mandate, subscription of financieel record aan.
