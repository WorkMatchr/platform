...
| `TwoFactor` | Native Better Auth 1.6.23-record voor TOTP en recovery codes. | Verplichte `User`-FK met indexen op `userId` en het Better Auth-vereiste `secret`; verificatie-, mislukte-pogingen- en lockstate volgen Better Auth. | TOTP-secret en recovery-codeopslag zijn strikt geheim. Recovery codes worden versleuteld opgeslagen; trusted devices zijn niet actief in WorkMatchr. |
...
| `FinancialInvoiceCounter`, `FinancialInvoice` | Globale nummerreeks en immutable factuur-/creditnotasnapshot, inclusief de overgenomen `pricingMode`. | Eén zakelijke bron per document; correcties maken een nieuw document en tonen altijd de werkelijk betaalde bedragen. |
