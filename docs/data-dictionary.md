...
| `PublicIntakeDraft` | Actuele pseudonieme conceptintake vóór authenticatie met fase, entrypoint, immutable broninvoer, flowversie, stap, optimistic version en interactie-/expirytijden. | Geen User- of Organization-FK; één sessie; unieke actuele antwoorden per vraag; indexen op fase/inactiviteit en expiry. `ABANDONED_BY_USER` is terminaal. | Vrije tekst kan persoonsgegevens bevatten; bewuste beëindiging verwijdert niets. `ABANDONED_TIMEOUT` en `EXPIRED` zijn gereserveerd voor latere gecontroleerde processen. |
