...
| `TwoFactor` | Historisch record uit de voormalige Better Auth-2FA-integratie. | Verplichte `User`-FK met bestaande indexen; geen actieve applicatieflow leest of schrijft dit model. | Historische factor- en recoverydata blijft inert voor audit-/retentiedoeleinden; verwijderen of wijzigen is geen onderdeel van de productflow. |
...
| `PublicIntakeAnswer` | Actuele getypeerde waarde of expliciete status `UNKNOWN`/`SKIPPED`, inclusief server-side vastgestelde bron. | Uniek draft/vraag; exact één passende scalarwaarde of bij `MULTI_OPTION` één niet-lege beheerde codelijst bij `ANSWERED`; oplopende versie; bron kan ook `AI_CONTEXT_PLANNER` zijn. | Mutaties lopen uitsluitend via de service en schrijven atomair een revisie. Vrije displaytekst is nooit de enige betekenisdrager van een kennisgestuurd antwoord. |
...
| `PublicIntakeContextQuestion` | Immutable, door de contextplanner geselecteerde vervolgvraag voor één publieke draft. | Uniek per draft/vraag en draft/sequence. `contextGoalCode` is een stabiele semantische code; `planningSnapshot` bevat engineversie, mode, reason code, score, knowledge-provenance en beheerde opties. Historische records zonder v1-provenance blijven geldig. | Geen vrije hulpvraag, token of secret dupliceren. Nieuwe dynamische opties worden bij beantwoording server-side tegen deze snapshot gevalideerd. |
...
| `PublicIntakeAbuseBucket` | Persistente, environment-gescheiden begrenzing voor publieke intakeverzoeken en externe AI-classificaties. | Uniek op environment, operatie, subjecttype, HMAC-subjecthash en vensterstart; positieve teller; begrensd venster en expiry. | Bevat nooit een ruw IP-adres, sessietoken, hulpvraag of providerantwoord. Verlopen buckets worden tijdens limitergebruik verwijderd. |
...
| `AssignmentProviderSelection` | Herleidbare reguliere providerselectie. | Uniek assignment/provider; score 0–100; bron/status/datumindexen. | Nooit stilzwijgend verwijderen; actieve selecties blijven binnen `Assignment.maxSelections`. |
| `Assignment.maxSelections` | Effectieve offerteplaatslimiet voor één Opdracht. | Verplicht; default `3`; databasecheck `3..5`. | Fase 1 kan alleen `3` publiceren; `4` of `5` vereist later aantoonbaar betaalbewijs. |
...
| `AdminCommunication` | Exacte tekst- en optionele HTML-snapshot van een gewone beheerdermail. | Auteur-FK, doelcontext, unieke dispatchsleutel en koppeling vanuit `AdminActionLog`. | Immutable; alleen OWNER/ADMIN kan inhoud server-side lezen. Securitymails en tokens zijn uitgesloten. |
| `AdminCommunicationDeliveryAttempt` | Append-only provideracceptatie- of foutpoging per beheercommunicatie. | Unieke communicatie/poging, veilige transport-, status- en foutvelden. | Immutable; provideracceptatie is geen afleverbevestiging. |
...
| `FinancialPurchase`, `FinancialPaymentEvent`, `FinancialRefund`, `FinancialEvent` | Immutable aankoopbasis, append-only betaalstatussen en gecontroleerde WorkMatchr-terugbetaling. `FinancialPurchase.pricingMode` onderscheidt de normale prijs van de strikt begrensde Mollie-sandboxacceptatieprijs. Refundevents verwijzen via `FinancialEvent.refundId` naar hun refund. | Bedragen in eurocenten; `EUR`; pending reserveert uitsluitend, alleen server-side geverifieerd `refunded` activeert ledgercorrectie en creditnota. De testprijs is databasebreed beperkt tot 25 credits voor €1,00 exclusief btw. |
| `FinancialInvoiceCounter`, `FinancialInvoice` | Globale nummerreeks en immutable factuur-/creditnotasnapshot, inclusief `pricingMode`, `snapshotVersion` en voor v2 de lever-, vooruitbetalings- en dienstperiodedatums. | Bestaande documenten blijven v1; nieuwe reguliere facturen zijn fail-closed v2. Correcties maken een nieuw document. |
| `FinancialInvoiceLine` | Geordende immutable v2-factuurregels met hoeveelheid, eenheid, eenheidsprijs, bruto, korting, netto, btw en optionele dienstperiode. | Uniek per factuur/positie; `UPDATE`/`DELETE` geblokkeerd en `ON DELETE RESTRICT`. |
| `FinancialInvoiceVatSummary` | Immutable v2-btw-samenvatting per toegepast tarief. | Uniek per factuur/tarief; deferred totaliteitscontrole tegen regels en hoofdtotaal. |
...
| `KnowledgeClaim.usageScopes` | Expliciete lijst met toegestane runtimegebruiksscenario's voor een claim. | Intakevragen en expertiserouting accepteren uitsluitend `INTAKE_ROUTING_KNOWLEDGE`; publicatie alleen verleent geen gebruiksrecht. |
| `KnowledgeRule.usageScopes` | Expliciete runtimebegrenzing van een contextdoel- of expertiseroutingregel. | Lege of afwijkende scope faalt gesloten en wordt niet door de intake-engine geladen. |
| `PublicIntakeDraft.caseUnderstandingJson` | Versiegestuurde semantische structurering van de vrije hulpvraag, inclusief waarde, bewijs, confidence en status per element. | Hypotheses blijven expliciet hypotheses; onbekenden en bewijs worden behouden. Geen zelfstandig juridisch, medisch of veiligheidsbesluit. |
| `PublicIntakeDraft.matchingProfileJson` | Beheerd expertiseroutingprofiel met primaire/conditionele expertise, specialismen en matchingcodes. | Alleen beheerde WorkMatchr-codes mogen matching sturen; vrije AI-tekst creëert geen taxonomiecode. |
