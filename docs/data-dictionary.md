...
| `FinancialJorttSync`, `FinancialJorttSyncAttempt` | Downstream boekhoudprojectie met Jortt remote ID, administratief factuurnummer, unieke technische invoice-reference, retry-status en immutable pogingen. | `technicalReference` gebruikt `workmatchr-invoice:<FinancialInvoice.id>` als conflictsleutel; het `WM-...`-nummer blijft de menselijke Jortt-reference. Een synchronisatiefout wijzigt kernrecords nooit. |
| `FinancialMaintenanceRun` | Begrensde operationele registratie van de bestaande financiële maintenance-orchestratie. | Bevat uitsluitend tijden, trigger, status, categorietotalen en veilige foutcodes. Een gedeeltelijke unieke index staat maximaal één actuele `RUNNING`-run toe. |
