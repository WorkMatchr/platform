...
| `Assignment` | Concrete opdracht met oplopende `version`. | Verplichte client/creator; unieke optionele intake, locatie, sector en primair specialisme; status/datumindexen. | Intakekoppeling is na vorming immutable; `archivedAt` of status; omschrijving kan gevoelige bedrijfsinformatie bevatten. |
| `AssignmentStatusHistory` | Zakelijke historie van opdrachtstatusovergangen. | Opdracht, oude/nieuwe status, actor, reden en tijdstip. | Append-only; initieel `null → DRAFT`. |
| `AssignmentRevision` | Volledige snapshot van zakelijke opdrachtvelden per versie. | Uniek opdracht/versie; actor en optionele locatie-, sector- en specialismerelaties. | Append-only en sluit aan op de actuele `Assignment.version`; AVG-bewaartermijn nog vaststellen. |
