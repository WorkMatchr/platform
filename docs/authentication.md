...
De ingelogde werkruimte toont de actuele gebruiker en actieve organisatie uit dezelfde servercontext als de beschermde pagina. Op desktop staat de accountnavigatie in een linkerzijbalk met de inklapbare groepen `Werk`, `Organisatie` en `Persoonlijk`; op mobiel staat exact dezelfde informatiearchitectuur in een compact accountmenu. Geldige rolgebonden items, waaronder providerdossier, professionals en financiële providerfuncties, worden uitsluitend uit de bestaande server-side account- en organisatiecontext afgeleid. De actieve groep opent automatisch en de meest specifieke actieve route wordt gemarkeerd. Na logout volgt een volledige navigatie naar de publieke homepage; er is geen organisatiewisselaar.
