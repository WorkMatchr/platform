...
- Platformbeheer is een afzonderlijke App Router-grens onder `/platformbeheer`. Layout, pagina’s, queries, Server Actions en exports controleren elk server-side de volledige centrale platformidentiteit: actief account, `PlatformRole.ADMIN` en actieve membership bij de actieve `PLATFORM_OPERATOR` met `systemKey = WORKMATCHR_PLATFORM`.
