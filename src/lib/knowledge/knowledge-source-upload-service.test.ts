...
vi.mock('./knowledge-extractor', () => ({
  extractPdfFullSource: vi.fn(async () => ({
    ...
    pages: [{ pageNumber: 1, status: 'TEXT_EXTRACTED', textHash: 'b'.repeat(64), ocrUsed: false, confidence: null,
      blocks: [{ pageNumber: 1, blockType: 'PARAGRAPH', blockOrder: 0, text: 'Veilige inhoud', textHash: 'a'.repeat(64), metadata: {} }] }],
...
const database = (duplicate: object | null = null) => ({ knowledgeSourceVersion: { findFirst: vi.fn(async () => duplicate) } })
...
describe('Knowledge Source Upload v1', () => {
  ...
  it('analyseert een geldige PDF deterministisch als metadatareview', async () => {
    ...
    expect(result).toMatchObject({ status: 'NEEDS_METADATA_REVIEW', proposedTitle: 'veilige bron', pageCount: 1, blockCount: 1 })
  ...
  it('weigert import zonder expliciete bevestiging en met ontbrekende metadata', async () => {
    ...
    await expect(confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: false, actorUserId: 'actor', storage, database: {} as never })).rejects.toMatchObject({ code: 'EXPLICIT_CONFIRMATION_REQUIRED' })
    await expect(confirmKnowledgeSourceUpload({ preview, metadata: { ...metadata, publisher: '' }, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })).rejects.toMatchObject({ code: 'METADATA_REQUIRED' })
  })

  it('importeert na bevestiging via de bestaande atomische conceptstraat met auditcontext', async () => {
    const storage = new InMemoryKnowledgeSourceUploadStorage()
    const preview = await analyzeKnowledgeSourceUpload({ bytes: pdf, fileName: 'bron.pdf', mediaType: 'application/pdf', storage, database: database() as never })
    const result = await confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })
  ...
  it('weigert een mogelijke duplicate fail-closed', async () => {
    ...
    await expect(confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })).rejects.toMatchObject({ code: 'POSSIBLE_DUPLICATE' })
  })
  ...
  it.each(['SOURCE_VERSION_CONFLICT', 'SOURCE_IDENTITY_CONFLICT'])('laat %s fail-closed door de bestaande onboardingstraat afhandelen', async (code) => {
    ...
    await expect(confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })).rejects.toMatchObject({ code })
  })
  ...
  it('laat een identieke bevestigde replay idempotent door de bestaande ingest hergebruiken', async () => {
    ...
    const first = await confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })
    const replay = await confirmKnowledgeSourceUpload({ preview, metadata, explicitlyConfirmed: true, actorUserId: 'actor', storage, database: {} as never })
