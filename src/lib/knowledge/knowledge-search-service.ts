...
export async function searchKnowledge(input: KnowledgeSearchInput) {
  ...
  return getPrisma().knowledgeClaim.findMany({
    where: {
      ...
      ...(input.query?.trim() ? { OR: [
        { statement: { contains: input.query.trim(), mode: 'insensitive' } },
        { normalizedStatement: { contains: input.query.trim(), mode: 'insensitive' } },
        { topic: { title: { contains: input.query.trim(), mode: 'insensitive' } } },
      ] } : {}),
    },
    select: {
...
export async function getKnowledgeAdminOverview() {
  ...
  const [
    ...
    database.knowledgeClaim.findMany({ where: { temporalStatus: { notIn: ['HISTORICAL', 'SUPERSEDED'] } }, select: { id: true, externalKey: true, statement: true, temporalStatus: true, validationStatus: true, publicationStatus: true, accessTier: true, controlRisk: true, sourceControlStatus: true, topic: { select: { title: true } }, citations: { select: { sourceVersion: { select: { versionLabel: true, source: { select: { code: true } } } }, fragment: { select: { pageFrom: true, pageTo: true, sectionPath: true } } }, take: 1 } }, orderBy: { createdAt: 'desc' }, take: 50 }),
