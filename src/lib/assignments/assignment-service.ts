...
function parseInput<T>(schema: { safeParse(value: unknown): { success: true; data: T } | { success: false; error: { flatten(): { fieldErrors: Record<string, string[] | undefined> } } } }, value: unknown): T {
...
export async function updateAssignment(
  ...
  const input = parseInput(assignmentEditSchema, rawInput)
  return getPrisma().$transaction(async (transaction) => {
    ...
    assertVersion(assignment.version, input.expectedAssignmentVersion)
    await validateLocation(transaction, organizationId, input.locationId, input.allowsRemoteWork)
...
async function transitionAssignment(
  ...
  return getPrisma().$transaction(async (transaction) => {
    ...
    if (toStatus === 'READY_FOR_REVIEW') {
      ...
      await validateLocation(transaction, organizationId, assignment.locationId, assignment.allowsRemoteWork)
    }
    const updated = await transaction.assignment.updateMany({
...
export function markAssignmentReadyForReview(userId: string, organizationId: string, rawInput: AssignmentTransitionInput) {
  const input = parseInput(assignmentTransitionSchema, rawInput)
...
export function reopenAssignment(userId: string, organizationId: string, rawInput: AssignmentReasonTransitionInput) {
  const input = parseInput(assignmentReasonTransitionSchema, rawInput)
