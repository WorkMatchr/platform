...
const statusLabels = { INVITED: 'Uitgenodigd', ACTIVE: 'Actief', BLOCKED: 'Geblokkeerd', ARCHIVED: 'Gearchiveerd', DELETION_PENDING: 'Verwijdering gepland', ANONYMIZED: 'Geanonimiseerd' } as const
...
export default async function PlatformUsersPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const filters: UserListFilters = {
    query: params.q,
    status: Object.keys(statusLabels).includes(params.status ?? '') ? params.status as UserListFilters['status'] : undefined,
    role: Object.keys(roleLabels).includes(params.role ?? '') ? params.role as UserListFilters['role'] : undefined,
  }
  ...
  return (
    <>
      ...
      <FilterForm>
        ...
        <FilterField name="status" label="Accountstatus"><select className="min-h-10 rounded-control border border-border px-3" name="status" defaultValue={filters.status ?? ''}><option value="">Alle statussen</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></FilterField>
        <FilterField name="role" label="Organisatierol"><select className="min-h-10 rounded-control border border-border px-3" name="role" defaultValue={filters.role ?? ''}><option value="">Alle rollen</option>{Object.entries(roleLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></FilterField>
      </FilterForm>
        ...
        <AdminTable headers={['Gebruiker', 'Organisatie', 'Rol', 'Status', 'Laatste sessie', 'Lifecycle']}>
          {users.map((user) => {
            ...
            return <tr key={user.id}>
              ...
              <td className="px-4 py-3">{membership ? roleLabels[membership.role] : user.platformRole === 'ADMIN' ? 'Platformbeheerder' : 'Geen organisatierol'}</td>
              <td className="px-4 py-3"><StatusPill tone={user.status === 'ACTIVE' ? 'good' : user.status === 'BLOCKED' ? 'bad' : 'warning'}>{statusLabels[user.status]}</StatusPill></td>
