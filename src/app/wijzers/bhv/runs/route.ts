...
export async function POST(request: Request) {
  try {
    ...
    const report = buildBhvReportData({ answers, organizationName: context.activeMembership.organization.name, scannedAt: completedAt, tier: 'BASIC' })
    const result = await completeArboGuideRun({ guideType: 'BHV', guideVersion: String(BHV_GUIDE_VERSION), reportVersion: BHV_REPORT_VERSION, organizationId: context.activeMembership.organization.id, completedByUserId: context.user.id, idempotencyKey: body.idempotencyKey, startedAt, completedAt, answersSnapshot: answers, reportSnapshot: report })
    return Response.json({ saved: true, runId: result.id, reportNumber: result.reportNumber }, { status: result.created ? 201 : 200 })
  } catch (error) {
    const status = error instanceof ArboGuideRunError && error.code === 'CONFLICT' ? 409 : 400
