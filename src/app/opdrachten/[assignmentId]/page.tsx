...
import { cancelAssignmentAction, reopenAssignmentAction, withdrawPublishedAssignmentAction } from '@/app/opdrachten/actions'
...
export default async function AssignmentDetailPage({ params, searchParams }: { params: Promise<{ assignmentId: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
      <Section spacing="compact" containerSize="narrow">
        ...
        <div className="mt-8"><AssignmentDetail assignment={assignment} actions={{ reopen: reopenAssignmentAction, cancel: cancelAssignmentAction, withdraw: withdrawPublishedAssignmentAction }} /></div>
      </Section>
    )
}
