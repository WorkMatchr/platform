...
import { quoteStatusLabels } from '@/lib/marketplace/marketplace-presentation'
  ...
  return (
    <Section spacing="compact">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <Heading as="h1" size="h2">Offertes vergelijken</Heading>
          <p className="mt-3 text-text-secondary">{assignment.title}. U ziet alleen offertes voor uw eigen opdracht.</p>
        </div>
        {assignment.status === 'OPEN' ? (
          <Link className="font-semibold underline" href={`/opdrachten/${assignment.id}/selectie`}>Selectie starten</Link>
        ) : null}
      </div>
      <div className="mt-8 grid gap-5 lg:grid-cols-2">
        {assignment.marketplaceQuotes.map((quote) => quote.submittedVersion ? (
          <Card key={quote.id}>
            <h2 className="text-xl font-semibold">{quote.providerOrganization.name}</h2>
            <Link className="mt-2 inline-block font-semibold underline" href={`/opdrachten/${assignment.id}/dienstverleners/${quote.providerProfileId}`}>Dienstverlenersprofiel bekijken</Link>
            <p className="mt-3 text-2xl font-bold">€ {(Number(quote.submittedVersion.priceCents) / 100).toLocaleString('nl-NL', { minimumFractionDigits: 2 })}</p>
            <p className="mt-3 whitespace-pre-wrap text-text-secondary">{quote.submittedVersion.approach}</p>
            <h3 className="mt-5 font-semibold">Planning</h3>
            <p className="mt-1 whitespace-pre-wrap">{quote.submittedVersion.planning}</p>
        ...
        {assignment.marketplaceQuotes.length === 0 ? (
          <Card>
            <p>Er zijn nog geen ingediende offertes.</p>
            {assignment.status === 'OPEN' ? (
              <Link className="mt-4 inline-block font-semibold underline" href={`/opdrachten/${assignment.id}/selectie`}>Start de selectie</Link>
            ) : null}
          </Card>
        ) : null}
      </div>
