...
export default async function ProviderControlPage() {
  ...
  return <><ProviderPageHeader title="Controleren en indienen" description="Controleer de volledigheid, los open acties op en dien vervolgens een vastgezette versie van uw dossier in voor beoordeling." readOnly={!canManage} /><ProviderStatusSummary statuses={control.statuses} /><ProviderCompleteness assessment={control.completeness} /><ProviderOpenActions actions={control.openActions} canManage={canManage} />
    {control.latestSubmission && <Card className="mt-6"><h2 className="font-bold">Laatste indiening</h2><p className="mt-2 text-sm text-text-secondary">Status: {providerReviewLabels[control.latestSubmission.status]} | versie van het ingediende dossier {control.latestSubmission.candidateVersion ?? 'onbekend'} | ingediend op {new Date(control.latestSubmission.submittedAt).toLocaleString('nl-NL')}</p></Card>}
    {canManage && control.isSubmittable && <Card className="mt-6"><h2 className="mb-2 text-lg font-bold">Dossier indienen</h2><p className="mb-5 text-sm text-text-secondary">Na indienen beoordeelt WorkMatchr uitsluitend de vastgezette ingediende versie. Goedkeuring maakt u niet automatisch gekwalificeerd of selecteerbaar.</p><ProviderSubmitForm action={submitProviderDossierAction} profileVersion={control.profileVersion} /></Card>}
