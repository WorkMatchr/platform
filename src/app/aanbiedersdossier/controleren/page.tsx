...
import { getProviderDossierControlView, getProviderDossierDashboard } from '@/lib/providers/provider-dossier-query-service'
...
export default async function ProviderControlPage() {
  ...
  const [control, dashboard] = await Promise.all([getProviderDossierControlView(context.user.id, context.providerProfileId), getProviderDossierDashboard(context.user.id, context.providerProfileId)])
