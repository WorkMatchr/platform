...
export default async function ProviderInsurancePage() {
  ...
  return <><ProviderPageHeader title="Verzekeringen" description="Leg verzekeringsgegevens vast. Alleen veilig gecontroleerde bewijsstukken kunnen worden gekoppeld." readOnly={!editable} /><div className="space-y-4">{data.value.map((item) => <Card key={item.id}><div className="flex justify-between gap-4"><div><h2 className="font-bold">{item.revisions[0]?.insuranceTypeTerm.label}</h2><p className="text-sm text-text-secondary">{item.revisions[0]?.insurer} · geldig tot {item.revisions[0]?.expiresAt.toLocaleDateString('nl-NL')}</p></div>{editable && <ProviderRecordStatusForm action={setProviderRecordStatusAction} profileVersion={dashboard.profileVersion} recordVersion={item.version} recordId={item.id} kind="INSURANCE" status={item.status} returnPath="/aanbiedersdossier/verzekeringen" />}</div></Card>)}</div>{editable && options.evidence.length > 0 ? <Card className="mt-6"><h2 className="mb-5 text-lg font-bold">Verzekering toevoegen</h2><ProviderInsuranceForm action={saveProviderInsuranceAction} profileVersion={dashboard.profileVersion} insuranceTypes={options.insuranceTypes} evidence={options.evidence} /></Card> : editable && <div className="mt-6"><ProviderFailureNotice title="Bewijsstukken uploaden is nog niet beschikbaar">Er kan nu geen veilig gecontroleerd bewijsstuk worden gekoppeld. U kunt de verzekeringsgegevens wel bewaren en het bewijsstuk later toevoegen.</ProviderFailureNotice></div>}</>
}
