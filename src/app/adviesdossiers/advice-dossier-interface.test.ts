...
describe('Adviesdossier-interface en beveiliging', () => {
  ...
  it('start vanuit een gereed dossier uitsluitend de bestaande opdrachtintake', () => {
    ...
    expect(actions).toContain('executeAdviceDossierIntakeHandoff')
    expect(actions).toContain('prepareAdviceDossierIntakeAction')
    expect(actions).toContain('redirect(result.href)')
    expect(actions.match(/startAdviceDossierIntake\(\{ viewer, dossierId \}\)/g)).toHaveLength(1)
  })
})
