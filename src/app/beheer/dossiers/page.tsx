...
import { presentProviderPermission, providerReviewLabels } from '@/lib/providers/provider-dossier-presentation'
...
export default async function ProviderReviewWorklistPage() {
  ...
  return <Section spacing="compact"><Heading as="h1" size="h2">Dossiers voor beoordeling</Heading><p className="mt-3 text-text-secondary">De beoordeling leest uitsluitend de vastgezette ingediende versie. Uw bevoegdheden: {worklist.permissions.map(presentProviderPermission).join(', ')}.</p><div className="mt-8 grid gap-4">{worklist.submissions.map((submission) => <Card key={submission.id}><div className="flex flex-wrap items-center justify-between gap-4"><div><h2 className="font-semibold">{submission.providerProfile.organization.name}</h2><p className="mt-1 text-sm text-text-secondary">Status: {providerReviewLabels[submission.status]} · versie {submission.version}</p></div><Link className="font-semibold underline" href={`/beheer/dossiers/${submission.providerProfile.id}`}>Open dossier</Link></div></Card>)}{worklist.submissions.length === 0 ? <p className="text-text-secondary">Er staan geen dossiers in de beoordelingswachtrij.</p> : null}</div></Section>
}
