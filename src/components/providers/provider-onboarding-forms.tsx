...
export function ProviderQualificationForm({ action, profileVersion, professionalId, qualifications, capabilities }: { action: Action; profileVersion: number; professionalId: string; qualifications: Option[]; capabilities: Option[] }) {
  ...
  return <ActionForm data-provider-form={form.formId} action={form.formAction} onChange={() => form.setDirty(true)} className="space-y-5" noValidate>
    ...
    <div><label htmlFor="issuer" className="font-semibold">Opleider of uitgevende instantie</label><input id="issuer" name="issuer" defaultValue={form.value('issuer')} className={fieldClassName} aria-invalid={form.invalid('issuer')} aria-describedby="issuer-error" /><FieldError id="issuer-error" message={form.error('issuer')} /></div>
    <fieldset aria-describedby="isCertified-help isCertified-error"><legend className="font-semibold">Betreft dit een certificaat?</legend><p id="isCertified-help" className="mt-1 text-sm text-text-secondary">Deze keuze beschrijft het soort bewijs. De vermelding blijft door u opgegeven en wordt niet automatisch door WorkMatchr geverifieerd.</p><div className="mt-2 flex flex-wrap gap-4"><label className="flex min-h-11 items-center gap-2"><input type="radio" name="isCertified" value="true" defaultChecked={certified === 'true'} /><span>Ja</span></label><label className="flex min-h-11 items-center gap-2"><input type="radio" name="isCertified" value="false" defaultChecked={certified !== 'true'} /><span>Nee</span></label></div><FieldError id="isCertified-error" message={form.error('isCertified')} /></fieldset>
    <details className="rounded-control border border-border p-4">
      <summary className="cursor-pointer font-semibold">Aanvullende registratiegegevens</summary>
      <p className="mt-2 text-sm text-text-secondary">Vul deze gegevens alleen in wanneer ze bij deze opleiding, kwalificatie of registratie horen.</p>
      <div className="mt-4 space-y-4">
        <div><label htmlFor="registrationNumber" className="font-semibold">Registratie- of certificaatnummer <span className="font-normal text-text-secondary">(optioneel)</span></label><input id="registrationNumber" name="registrationNumber" maxLength={200} defaultValue={form.value('registrationNumber')} className={fieldClassName} /></div>
        <div className="grid gap-4 sm:grid-cols-2"><div><label htmlFor="issuedAt" className="font-semibold">Behaald op <span className="font-normal text-text-secondary">(optioneel)</span></label><input id="issuedAt" name="issuedAt" type="date" defaultValue={form.value('issuedAt')} className={fieldClassName} /></div><div><label htmlFor="validUntil" className="font-semibold">Geldig tot <span className="font-normal text-text-secondary">(optioneel)</span></label><input id="validUntil" name="validUntil" type="date" defaultValue={form.value('validUntil')} className={fieldClassName} /></div></div>
      </div>
    </details>
    ...
    <p className="text-sm text-text-secondary">Nieuwe kwalificaties worden opgeslagen als door u opgegeven.</p>
...
export function ProviderRecordStatusForm({ action, profileVersion, recordVersion, recordId, kind, status, returnPath }: { action: Action; profileVersion: number; recordVersion: number; recordId: string; kind: string; status: 'ACTIVE' | 'ARCHIVED'; returnPath: string }) {
  ...
  return <ActionForm action={form.formAction}><input type="hidden" name="expectedProfileVersion" value={profileVersion} /><input type="hidden" name="expectedRecordVersion" value={recordVersion} /><input type="hidden" name="recordId" value={recordId} /><input type="hidden" name="kind" value={kind} /><input type="hidden" name="status" value={next} /><input type="hidden" name="returnPath" value={returnPath} /><Button type="submit" variant="secondary" loading={form.pending}>{next === 'ARCHIVED' ? 'Archiveren' : 'Opnieuw activeren'}</Button></ActionForm>
}
