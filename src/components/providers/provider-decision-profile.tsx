    ...
    <div className="grid min-w-0 items-start gap-7 lg:grid-cols-[minmax(17rem,0.34fr)_minmax(0,0.66fr)]">
      <aside className="min-w-0 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto lg:overscroll-contain">
        <Card className="space-y-5">
          ...
          <div><div className="flex flex-wrap items-center gap-2"><h1 className="break-words text-2xl font-bold text-brand-dark">{profile.organization.tradeName ?? profile.organization.name}</h1>{profile.hasActivePro && <span aria-label="Actief WorkMatchr Pro-abonnement" className="inline-flex rounded-full border border-brand/30 bg-brand/10 px-2.5 py-1 text-xs font-bold text-brand-dark">WorkMatchr Pro</span>}</div>{profile.organization.tradeName && <p className="mt-1 text-sm text-text-secondary">{profile.organization.name}</p>}<p className="mt-2 text-sm text-text-secondary">{location?.city ?? 'Vestigingsplaats niet ingevuld'}{location?.province ? `, ${location.province}` : ''}</p></div>
