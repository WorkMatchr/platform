...
type Item = { id: string; file: File; state: 'WAITING' | 'ANALYZING' | 'READY' | 'ERROR' | 'IMPORTED'; preview?: KnowledgeSourceUploadPreview; metadata?: KnowledgeSourceUploadMetadata; message?: string; sourceVersionId?: string }
...
export function KnowledgeSourceUploadForm({ storageConfigured }: { storageConfigured: boolean }) {
    ...
    await Promise.all(Array.from({ length: Math.min(queue.length, KNOWLEDGE_SOURCE_UPLOAD_CONCURRENCY) }, async () => {
      for (;;) {
        ...
        try {
          ...
          analyzed.set(item.id, { ...item, preview, metadata: metadataFrom(preview), state: preview.status === 'DUPLICATE' ? 'ERROR' : 'READY', message: preview.status === 'DUPLICATE' ? 'Duplicaat: deze bronversie bestaat al.' : undefined })
  ...
  const importOne = (id: string) => startTransition(async () => {
    ...
    const result = await confirmKnowledgeSourceUploadAction({ preview: item.preview, metadata: item.metadata, explicitlyConfirmed: true, relationshipReviewed: true })
  ...
  const saveFamilies = () => startTransition(async () => {
    for (const family of families.filter((entry) => entry.enabled)) {
      const members = family.members.map((member, index) => ({ sourceVersionId: items.find((item) => item.preview?.checksum === member.checksum)?.sourceVersionId ?? '', role: member.role, sequence: index + 1 }))
  ...
  return <div className="space-y-6">
    ...
    {items.map((item) => item.preview && item.metadata ? <MetadataCard key={item.id} item={item} pending={pending} update={update} importOne={importOne} /> : null)}
...
function MetadataCard({ item, pending, update, importOne }: { item: Item; pending: boolean; update: (id: string, name: keyof KnowledgeSourceUploadMetadata, value: string) => void; importOne: (id: string) => void }) {
  ...
  return <section className="rounded-card border border-border bg-surface p-5"><div className="flex flex-wrap justify-between gap-2"><div><h2 className="font-bold text-brand-dark">Stap 2 — Automatisch voorgestelde metadata</h2><p className="break-all text-sm text-text-secondary">{item.file.name}</p></div><span className="rounded-full bg-warning/15 px-3 py-1 text-xs font-semibold">Menselijke controle nodig</span></div>
    <div className="mt-4 grid gap-4 lg:grid-cols-2">{input('Titel', 'title')}{input('Uitgever', 'publisher')}{input('Publicatiejaar', 'publicationYear')}{input('Versie', 'versionLabel')}{input('Broncode', 'sourceCode')}{input('Canonieke HTTPS-URL (optioneel)', 'canonicalUrl')}</div>
