...
export function KnowledgeImprovementHandlingForm({ reportId, reviewTaskId, version }: { reportId: string; reviewTaskId: string; version: number }) {
  const [rawState, action, pending] = usePendingAction(handleKnowledgeImprovementAction, initialKnowledgeImprovementHandlingState, () => ({ ...initialKnowledgeImprovementHandlingState, status: 'error' as const, message: ACTION_ERROR }))
  const state = normalizeKnowledgeActionState(rawState)
  return (
    <ActionForm action={action} className="grid min-w-64 gap-2">
