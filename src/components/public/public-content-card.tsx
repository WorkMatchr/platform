...
export function PublicContentCard({ title, description, href, linkLabel = 'Lees meer', status, headingLevel = 'h2' }: PublicContentCardProps) {
  return (
    <Card className="flex h-full min-w-0 flex-col shadow-none">
      ...
      <Heading as={headingLevel} size="h3" className={status ? 'mt-2' : ''}>{title}</Heading>
