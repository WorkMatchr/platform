...
export function KnowledgeDocumentPage({ document, breadcrumbs, showShortAnswer = false }: KnowledgeDocumentPageProps) {
  return (
    <PublicPageLayout breadcrumbs={breadcrumbs} eyebrow={document.type === 'SERVICE' ? 'Dienstverlening' : document.type === 'LEGAL_OBLIGATION' ? 'Wettelijke verplichting' : 'Uitleg'} title={document.title} description={document.summary}>
      <Section containerSize="narrow" spacing="compact" containerClassName="space-y-10 sm:space-y-12">
        ...
        <RelatedTopics topics={document.relatedTopics} />
        <SourceList sources={document.sources} />
        <LastReviewed date={document.lastReviewed} />
        <KnowledgeCallToAction content={document.callToAction} />
      </Section>
    </PublicPageLayout>
  )
}
