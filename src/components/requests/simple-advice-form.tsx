...
import { simpleAdviceSchema, simpleAdviceRouteSchema, simpleAdviceSummary, requestedExpertiseOptions, helpTopicLabels, outcomeLabels, locationLabels, startLabels, usesLocation } from '@/lib/requests/simple-advice-contract'

type Draft = Record<string, string> & { routeChoice: string }
const empty: Draft = { routeChoice: '', requestedExpertise: '', helpTopic: '', helpTopicOther: '', requestTitle: '', requestDescription: '', desiredOutcome: '', desiredOutcomeOther: '', workLocationMode: '', organizationLocationId: '', otherLocationCity: '', desiredStartMode: '', desiredStartDate: '' }
export function SimpleAdviceForm({ action, viewerId, locations = [] }: {
  ...
  const [v, setV] = useState<Draft>({ ...empty, organizationLocationId: locations[0]?.id ?? '' })
  ...
  const payload = { ...v, requestedExpertise: v.requestedExpertise || null, helpTopic: v.helpTopic || null, organizationLocationId: v.organizationLocationId || null, combinationModes: modes }
  useEffect(() => {
  ...
  const attrs = (key: string) => ({ id: key, name: key, value: v[key], onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => update(key, e.target.value), className: fieldClassName, 'aria-invalid': Boolean(error(key)), 'aria-describedby': error(key) ? `${key}-error` : undefined })
  ...
  const validate = () => {
    ...
    if (usesLocation({ workLocationMode: v.workLocationMode, combinationModes: modes }, 'ORGANIZATION') && viewerId && !locations.some(l => l.id === v.organizationLocationId) && step > 0) Object.assign(selected, { organizationLocationId: ['Kies een beschikbare organisatielocatie of kies Op andere locatie.'] })
    setErrors(selected)
  ...
  return <form ref={form} action={formAction} className="space-y-6 rounded-card border border-border bg-surface p-5 sm:p-8" noValidate onSubmit={e => { if (step !== 2 || !validate()) e.preventDefault() }}>
    ...
    {step === 1 && <>
      {text('requestTitle', 'Waar heeft u hulp bij?', 'text', 'Beoordeling machineveiligheid productielijn')}
      <div><label className="mb-2 block font-semibold" htmlFor="requestDescription">Beschrijf uw vraag of situatie</label><textarea {...attrs('requestDescription')} rows={5} maxLength={4000} /><p className="mt-1 text-sm text-text-secondary">Noem geen namen, medische gegevens of andere gevoelige persoonsgegevens.</p>{fieldError('requestDescription')}</div>
      ...
      {usesLocation({ workLocationMode: v.workLocationMode, combinationModes: modes }, 'ORGANIZATION') && <>{select('organizationLocationId', 'Organisatielocatie', Object.fromEntries(locations.map(l => [l.id, l.city])))}{!locations.length && <p className="text-sm text-text-secondary">Na inloggen kunt u uw organisatielocatie kiezen. U kunt ook Op andere locatie kiezen en de plaats invullen.</p>}</>}
      ...
      {v.desiredStartMode === 'SPECIFIC_DATE' && text('desiredStartDate', 'Gewenste startdatum', 'date')}
    </>}
